...
public class CursoBaseDAOImpl extends AbstractDAOImpl<CursoBase, Integer> implements CursoBaseDAO {
	...
	public CursoBase findCursoBaseByCodigoByPlan(String codigo, Integer idplan) {
		...
		try {
			query = getCurrentSession()
					.createQuery("from CursoBase where cursobCodigo = :codigo and plan.idplan = :idplan")
					.setMaxResults(1);
	...
	public CursoBase findCursoBaseByNombreByPlanNombre(String nombre, String nombrePlan) {
		...
		try {
			query = getCurrentSession()
					.createQuery("from CursoBase where cursobNombre = :nombre and plan.planNombre = :nomPlan")
					.setMaxResults(1);
			query.setParameter("nombre", nombre);
			query.setParameter("nomPlan", nombrePlan);
			cursoBase = (CursoBase) query.uniqueResult();
		} catch (HibernateException he) {
			he.printStackTrace();
		}
	...
	public List<CursoBase> findCursoBaseSinConjunto() {
		...
		try {
			query = getCurrentSession().createQuery(
					"select b FROM CursoConjunto c RIGHT JOIN c.cursoBase b  WHERE b.idcursoGeneral NOT IN (select b.cursoBase.idcursoGeneral from CursoConjunto b  ) ");
	...
	public List<CursoBase> obtenerCursosxCod(String valor, String filtro) {
		...
		try {
			query = getCurrentSession()
					.createQuery("select c from CursoBase as c where c." + filtro + " LIKE '%" + valor + "%'");
